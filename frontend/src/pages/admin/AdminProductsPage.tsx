...
export default function AdminProductsPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    setFormError('')
    if (!nameEn.trim()) { setFormError('Product name is required'); return }
    if (!basePrice || isNaN(Number(basePrice)) || Number(basePrice) < 0) { setFormError('Invalid base price'); return }
    for (const v of variants) {
      if (!v.sku.trim()) { setFormError('Each variant must have a SKU'); return }
    }
    ...
    try {
      await adminService.createProduct({
        name_id: nameEn.trim(),
        name_en: nameEn.trim(),
        description_id: descEn.trim() || undefined,
                  ...
                  <tr key={p.id} className="hover:bg-neutral-50/60 transition-colors">
                    ...
                    <td className="py-3 px-4">
                      <div className="flex items-center justify-end gap-1">
                        <button
                          onClick={() => handleDeleteClick(p.id, p.name_en)}
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-6">
              ...
              <section>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div>
                    <label className="label">Base Price (IDR) *</label>
                    ...
                    <div key={idx} className="p-4 rounded-xl border border-neutral-200 bg-neutral-50/50 space-y-3">
                      ...
                      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                        ...
                        <div>
                          ...
                          <input type="number" min="0" value={v.price_override} onChange={(e) => updateVariant(idx, 'price_override', e.target.value)} className="input py-2 text-sm" placeholder="Empty = base price" />
                        </div>
                        <div>
                    ...
                    <div key={idx} className="flex items-center gap-2">
                      <div className="flex-1">
                        <input
                           ...
                           placeholder={`Image URL ${idx + 1}${idx === 0 ? ' (primary)' : ''}`}
                        />
                      </div>
                      <div className="w-40">
                        <input
